...
public class DocumentList implements Serializable {
	...
	// the underlying lists of strings are rarely cloned, but the maps themselves
	// are duplicated when a recipe is duplicated.
	...
	/*
	 * public Feature.Type getValueType(String label) { if(label == null) { return
	 * Type.NOMINAL; }
	 * 
	 * if(type != null && label.equals(currentAnnotation)) { return type; } else {
	 * Feature.Type localType = guessValueType(label);
	 * if(label.equals(currentAnnotation)) { setClassValueType(localType); } return
	 * localType; } }
	 */

	/**
	 * Uses a sort of shoddy and roundabout catch-exception way of figuring out if
	 * the data type is nominal or numeric.
	 * 
	 * @return
	 */
		...
		for (String s : getPossibleAnn(label)) {
			if (!s.equals(emptyAnnotationString))
				try {
					...
					// System.err.println("DL 118: failed to parse "+s+" -- defaulting to nominal
					// for column "+label);
	...
	public DocumentList(List<String> filenames, Map<String, List<String>> texts, Map<String, List<String>> annotations,
			String currentAnnot) {
	...
	public DocumentList(Set<String> filenames) {
		...
		for (String filename : filenames) {
			...
			try {
				...
				if (!f.exists()) {
					f = new File(Workbench.dataFolder.getAbsolutePath(),
	...
	public String guessTextAndAnnotationColumns() {
		...
		if (currentAnnotation == null || textColumns.isEmpty()) {
			for (String s : annotationNames) {
				...
				if (!missingValues && currentAnnotation == null && values.size() > 1
						&& (values.size() < (this.getSize() / 10.0)) || values.size() < 6) {
	...
	public List<String> getAnnotationArray(String name) {
		...
		// else throw new NoSuchElementException("Document List "+this.getName()+" has
		// no column named "+name + ": "+allAnnotations.keySet());
		// System.err.println("Warning: Document List "+this.getName()+" has no column
		// named "+name + ": "+allAnnotations.keySet()+". Returning null.");
		return null;
	}
	...
	// use FeatureTable.getAnnotation or ExtractFeatureControl.getTargetAnnotation
	// instead
	...
	public List<String> getListForTextColumns(String name) {
		return textColumns.get(name);
	}
	...
	public void setCurrentAnnotation(String annot) {
		if (annot == currentAnnotation) // TODO: make sure this shortcut doesn't break anything expecting labelArray to
										// be populated
			return;
	...
	// TODO: use this in BuildModel and Chef and anywhere else we're translating a
	// recipe to a new document list
	...
	public boolean equals(DocumentList other) {
		...
		toReturn = (this.getTextColumnsAreDifferentiated() == other.getTextColumnsAreDifferentiated()) ? toReturn
				: false;
	...
	public DocumentList clone() {
		DocumentList newDocs = new DocumentList(new ArrayList(getFilenameList()),
				new TreeMap<String, List<String>>(getCoveredTextList()),
				new TreeMap<String, List<String>>(allAnnotations()), currentAnnotation);
